...
const int64 nChainStartTime = 1404965741; 
...
const string strMessageMagic = "Sovereign Signed Message:\n";
...
static const int64 nTargetSpacingWorkMax = 5 * nStakeTargetSpacing; // 2-hour
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Sovereign miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Sovereign
    bool fStrictPayToScriptHash = true; // Always active in Sovereign
    ...
    // ppcoin: fees are not collected by miners as in Sovereign
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Sovereign", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Sovereign";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:Sovereign genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0xc980279768882a7c416c23ac797fb5ae13d9a1878508aa2deffa52a05f36faad"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe6, 0xd1, 0xa2, 0xf5 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("SovereignMiner : proof-of-work not meeting target");

    //// debug print
    printf("SovereignMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SovereignMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("SovereignMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadSovereignMiner(void* parg);

static bool fGenerateSovereigns = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void SovereignMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("Sovereign-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateSovereigns || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateSovereigns) && !fProofOfStake)
                return;
        }
        ...
        printf("Running SovereignMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadSovereignMiner(void* parg)
{
    ...
    try
    {
        ...
        SovereignMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadSovereignMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadSovereignMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadSovereignMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateSovereigns(bool fGenerate, CWallet* pwallet)
{
    fGenerateSovereigns = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateSovereigns = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d SovereignMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadSovereignMiner, pwallet))
                printf("Error: NewThread(ThreadSovereignMiner) failed\n");
            Sleep(10);
        }
    }
}
